...
const Create = () => {
  ...
  useBackOnClose(`add-employee-status`, open, onOpen, onClose);
  const { req } = useRequest({
    id: "crud_employee_status",
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      const url = `/api/mamura/admin/master-data/employee-status`;
      const config = {
        url,
        method: "POST",
        data: payload,
...
const Edit = (props: any) => {
  ...
  const { l } = useLang();
  const { open, onOpen, onClose } = useDisclosure();
  useBackOnClose(`edit-employee-status`, open, onOpen, onClose);
  const { req } = useRequest({
    id: "crud_employee_status",
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      const url = `/api/mamura/admin/master-data/employee-status/${initialData?.id}`;
      const config = {
        url,
        method: "POST",
        data: payload,
...
const TableData = (props: any) => {
  ...
  const { req, loading: deleteLoading } = useRequest({
    id: `crud_employee_status-${dataId}`,
  });
    ...
    },
    {
      ...
      confirmation: (rowData: any) => ({
        ...
        confirmCallback: () => {
          ...
          const url = `/api/mamura/admin/master-data/employee-status/${rowData.originalData.id}/restore`;
          const config = {
            url,
            method: "POST",
          };
    ...
    },
    {
      ...
      confirmation: (rowData: any) => ({
        ...
        confirmCallback: () => {
          ...
          const url = `/api/mamura/admin/master-data/employee-status/${rowData.originalData.id}`;
          const config = {
            url,
            method: "DELETE",
          };
...
const MasterDataEmployeeStatusPage = () => {
  ...
  const dataState = useDataState({
    url: `/api/mamura/admin/master-data/employee-status`,
    method: "GET",
    payload: {
